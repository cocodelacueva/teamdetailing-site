...
$grid_content = $el_class = $width = $orderby = $order = $el_position = $isotope_item = $view_all_caption = $view_all_link = '';
...
while ( $my_query->have_posts() ) {
    ...
    if( $grid_template == 'filtered_grid' ) {
        ...
        $post_categories = wp_get_object_terms($my_query->post->ID, array_values($taxonomies));//$post_categories = wp_get_object_terms($my_query->post->ID, array_keys($taxonomies));
    ...
    if ( $grid_link != 'link_no' ) {
		...
		$link = '<a class="more-link" href="'. get_permalink($post_id) .'"'.$link_target.' title="'. sprintf( esc_attr__( '%s', 'js_composer' ), the_title_attribute( 'echo=0' ) ).'">'. __("Read more", "js_composer") .'</a>';
    }

    // Thumbnail logic
    if ( in_array($grid_layout, array('title_thumbnail_text', 'thumbnail_title_text', 'thumbnail_text', 'thumbnail_title', 'thumbnail', 'title_text', 'portfolio', 'portfolio-classic', 'portfolio-fancy-1', 'portfolio-fancy-2') ) ) {
    ...
    if ( $grid_link != 'link_no' ) {
        ...
        if ( $grid_link == 'link_post' ) {
            ...
            $link_image_start = '<a class="link_image" href="'.get_permalink($post_id).'"'.$link_target.' title="'.sprintf( esc_attr__( '%s', 'js_composer' ), the_title_attribute( 'echo=0' ) ).'">';
            $link_title_start = '<a class="link_title" href="'.get_permalink($post_id).'"'.$link_target.' title="'.sprintf( esc_attr__( '%s', 'js_composer' ), the_title_attribute( 'echo=0' ) ).'">';
		}
            ...
            if ( $p_video != "" ) {
                ...
                if(isset($p_img_large[0])) //this line added by ozythemes
					$p_link = $p_img_large[0];
            }
            $link_image_start = '<a class="link_image prettyphoto" href="'.$p_link.'"'.$link_target.' title="'.the_title_attribute('echo=0').'">';
            //$link_title_start = '<a class="link_title" href="'.get_permalink($post_id).'"'.$link_target.' title="'.sprintf( esc_attr__( 'Permalink to %s', 'js_composer' ), the_title_attribute( 'echo=0' ) ).'">';
	...
	// If grid layout is: Text + Title Addition by ozythemes
	else if ( $grid_layout == 'text_title' ) {
		if ( $content ) {
			$to_filter = '<div class="entry-content">' . $content . '</div>';
			$teasers .= apply_filters('vc_teaser_grid_content', $to_filter, array("grid_layout" => $grid_layout, "ID" => $post_id, "content" => $content, "media_link" => $p_link) );
		}
		if ( $post_title ) 	{
			//$to_filter = '<h2 class="post-title">' . $link_title_start . $post_title . $link_title_end . '</h2>';
			$to_filter = '<h5 class="post-title">' . $post_title . '</h5>';
			$teasers .= apply_filters('vc_teaser_grid_title', $to_filter, array("grid_layout" => $grid_layout, "ID" => $post_id, "title" => $post_title, "media_link" => $p_link) );
		}
	}			
	// If grid layout is: Portfolio & New Listing Addition by ozythemes
	else if ( $grid_layout == 'portfolio' || $grid_layout == 'portfolio-classic' ) {
	
		if ( $thumbnail ) {
			wp_enqueue_script( 'prettyphoto' );
			wp_enqueue_style( 'prettyphoto' );					

			if (isset($p_video) && $p_video != "" ) {
				$p_link = $p_video;
			} else {
				$p_link = $p_img_large[0];
			}
			...
			if ( $grid_link === 'link_no' ) {
				$to_filter = '<div class="post-portfolio-icons"><a class="link_image prettyphoto icon-search" rel="prettyPhoto" href="'.$p_link.'"'.$link_target.' title="'.$post_title.'"></a></div>';
			}else{
				$to_filter = '<div class="post-portfolio-icons"><a class="link_image icon-plus" href="'.get_permalink($post_id).'"'.$link_target.'></a><a class="link_image prettyphoto icon-search" rel="prettyPhoto" href="'.$p_link.'"'.$link_target.' title="'.$post_title.'"></a></div>';
			}
			
			$to_filter .= '<div class="post-portfolio">' . $link_image_start . $thumbnail . $link_image_end .'</div>';
			$teasers .= apply_filters('vc_teaser_grid_thumbnail', $to_filter, array("grid_layout" => $grid_layout, "ID" => $post_id, "thumbnail" => $thumbnail, "media_link" => $p_link) );
		}
		if ( $post_title ) 	{
			$category_label = strip_tags( get_the_term_list( $post_id, 'portfolio_category', '', ', ', '' ) );
			
			$to_filter = '<div class="post-portfolio-title"><span class="category-label">'.$category_label.'</span><h2 class="post-title">' . $link_title_start . $post_title . $link_title_end . '</h2></div>';
			$teasers .= apply_filters('vc_teaser_grid_title', $to_filter, array("grid_layout" => $grid_layout, "ID" => $post_id, "title" => $post_title, "media_link" => $p_link) );
		}
		
		wp_register_style('portfolio-ozy', get_template_directory_uri().'/css/portfolio.css');
		wp_enqueue_style('portfolio-ozy');
	}            
	// If grid layout is: Portfolio & New Listing Addition by ozythemes
	else if ( $grid_layout == 'portfolio-fancy-1') {
		...
		if ( $thumbnail ) {
			...
			//$to_filter = '<div class="post-portfolio">' . $link_image_start . $thumbnail . $link_image_end; //not functioning on touch devices
			$to_filter = '<div class="post-portfolio">' . $thumbnail ;
			...
			if ( $post_title ) 	{
				...
				$to_filter = '<div class="post-portfolio-title"><span class="category-label">'.$category_label.'</span><h3 class="post-title">' . $link_title_start . $post_title . $link_title_end . '</h3></div>';
				$teasers .= apply_filters('vc_teaser_grid_title', $to_filter, array("grid_layout" => $grid_layout, "ID" => $post_id, "title" => $post_title, "media_link" => $p_link) );
			}
		...
		wp_register_style('portfolio-ozy', get_template_directory_uri().'/css/' . $grid_layout . '.css');
		wp_enqueue_style('portfolio-ozy');				
	}// If grid layout is: Portfolio & New Listing Addition by ozythemes
	else if ( $grid_layout == 'portfolio-fancy-2') {
		...
		if ( $thumbnail ) {
			...
			if ( $post_title ) 	{
				...
				$to_filter = '<div class="post-portfolio-title"><span class="category-label">' . $category_label . '</span><h3 class="post-title">' . $link_title_start . $post_title . $link_title_end . '</h3>';
				if ( $grid_link === 'link_no' ) {
					$to_filter .= '<div class="post-portfolio-icons"><a class="link_image prettyphoto icon-search" rel="prettyPhoto" href="'.$p_link.'"'.$link_target.' title="'.$post_title.'"></a></div>';
				}else{
					$to_filter .= '<div class="post-portfolio-icons"><a class="link_image icon-plus" href="'.get_permalink($post_id).'"'.$link_target.'></a><a class="link_image prettyphoto icon-search" rel="prettyPhoto" href="'.$p_link.'"'.$link_target.' title="'.$post_title.'"></a></div>';
...
if( $grid_template == 'filtered_grid' && $teasers && !empty($teaser_categories)) {
    ...
    */
    $categories_array = get_terms(array_values($taxonomies), array( //$categories_array = get_terms(array_keys($taxonomies), array(
        'orderby' => 'name',
        'include' => implode(',', $teaser_categories)
    ));
...
if ( $grid_template == 'carousel' ) {
	...
	if(($view_all_caption && $view_all_link) && trim($title) === "") { //added by ozythemes
		$output .= '<a href="' . $view_all_link . '" class="teaser-grid-carousel-view-all">' . $view_all_caption . '</a>';
	}
}
...
//Pagination addition by ozythemes
if(function_exists('get_pagination')) {
	if ( $grid_template !== 'carousel' ) {
		global $wp_query;
		$wp_query = null; $wp_query = $my_query;
